...
void print_matrix_debug(double* mat, int n) {
  for (int i=0; i<n; ++i) {
    for (int j=0; j<n; ++j) {
      printf("%lf ", mat[i*n+j]);
    }
...
void print_vector_debug(double* vec, int n) {
  for (int i=0; i<n; ++i) {
    printf("%lf ", vec[i]);
  }
  printf("\n");
...
char *validate_vect(char *msgheader, double global_x[], int n, int t,
  ...
  // printf("EXPECTED:\n");
  // print_vector_debug(expected, n);
  // printf("RESULT:\n");
  // print_vector_debug(global_x, n);
 ...
 *    A[i,i]=0 for diagonal elements. A[i,j]=-1/n for non-diagonal elements.
 * When matrix_type is UPPER_TRIANGULAR matrix  A of size nxn:
 *    A[i,i]=0 for diagonal elements. A[i,j]=-1/n for upper diagonal elements
...
int init_matrix(double *local_A, double *local_x, double *local_d,
  ...
  if (matrix_type == UPPER_TRIANGULAR) {
    ...
    for (int i=0; i<blocksize; ++i) {
      for (int j=0; j<n; ++j) {
        if (my_rank * blocksize + i == j) { // diagonal
          local_A[i*n+j] = 0.0;
        }
        else { // non-diagonal
          local_A[i*n+j] = -1.0 / n;
  ...
  // printf("D DEBUG %d: ", my_rank);
  // print_vector_debug(local_d, n);
